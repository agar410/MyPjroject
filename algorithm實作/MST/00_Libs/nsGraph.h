...
public:
	...
	GNode(int pId, double pWeight=1, int pColor=0, string pLabel="*") {//這裡和insertEdge中的e_vu有關  w的1去掉
	...
	void setAll(int pId=-1, double pWeight=1, int pColor=0, string pLabel="", int pre_point=-1,int pre_d=-1 ) {
		id = pId;
		weight = pWeight;
		color = pColor;
		label = pLabel;
...
void insertEdge( int u, int v, double w=0 ) {//w=0 的0去掉
